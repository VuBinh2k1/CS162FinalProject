...
int npscoreboard::staff(const char* course_id, const char* course_cs, int first_choose) {
...
LAYOUT:
	...
	UN_CHANGE:
		...
		if (c == KEY_SAVE) { npscoreboard::save(course_id, course_cs); goto LAYOUT; }
		if (c == 224 || c == 0) {
			c = getch();
...
int npscoreboard::lecturer(const char* course_id, const char* course_cs, int first_choose, int first_overflow) {
	std::ifstream inp(".\\layout\\minibox.layout");
	if (!inp.is_open()) {
		MessageBox(NULL, TEXT("minibox.layout is not exist"), TEXT("error layout"), MB_OK);
		return 0;
...
LAYOUT:
	...
	int choose = first_choose, cur = -1, overflow = first_overflow, editrow = 0;
	while ((cur = -1)) {
		csv_file process(PROCESS(course_id, course_cs), def_process);
		for (int i = 0; i < process.count; ++i) {
			csv_line* student = &process.data[i];
	...
	UN_CHANGE:
		...
		if (KEY_EDIT(c)) { npscoreboard::edit(course_id, course_cs, 10 + choose + overflow, editrow); goto LAYOUT; }
		if (c == KEY_OPEN) { npscoreboard::open(course_id, course_cs); goto LAYOUT; }
		if (c == 224 || c == 0) {
			c = getch();
...
void npscoreboard::save(const char* course_id, const char* course_cs) {
...
SAVE_AS:
	...
	std::string path;
	if (read(39, 16, 128, path, 48, SHOW) == KEY_ESC) return;
	gotoxy(32, 17, 128); std::cout << "                                                         ";
	if (path.empty()) goto SAVE_AS;
	if (path.back() != '\\') path += '\\'; path += ACADEMICYEAR + '-' + SEMESTER + '-' + course_id + '_' + course_cs + "_scoreboard.csv";
	...
	colorizing(134); std::cout << ACADEMICYEAR << '-' << SEMESTER << '-' << course_id << '_' << course_cs << "_scoreboard.csv";
	colorizing(129); std::cout << " success.";
	PAUSE;
}

void npscoreboard::open(const char* course_id, const char* course_cs) {
...
void npscoreboard::edit(const char* course_id, const char* course_cs, int y, int i) {
	...
	if (read(63, y, COLOR_CODE, mid, 4, SHOW, student->pdata[2]) == KEY_ESC) return;
	if (read(69, y, COLOR_CODE, lab, 4, SHOW, student->pdata[3]) == KEY_ESC) return;
	if (read(75, y, COLOR_CODE, bonus, 4, SHOW, student->pdata[4]) == KEY_ESC) return;
	if (read(81, y, COLOR_CODE, final, 4, SHOW, student->pdata[5]) == KEY_ESC) return;
	while (1) {

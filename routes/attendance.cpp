...
int npattendance::staff(const char* course_id, const char* course_cs, int first_choose) {
...
LAYOUT:
	...
	NO_CHANGE:
		...
		if (c == KEY_SAVE) { npattendance::save(course_id, course_cs); goto LAYOUT; }
		if (c == 224 || c == 0) {
			c = getch();
			if (c == KEY_UP && choose > 0) choose--;
			else if (c == KEY_DOWN && choose < cur - 17) choose++;
			else if (c == KEY_LEFT) return 1;
			else if (c == KEY_RIGHT) { if (npscoreboard::staff(course_id, course_cs, choose) == 0) return 0; goto LAYOUT; }
			else goto NO_CHANGE;
			continue;
		}
		goto NO_CHANGE;
...
int npattendance::lecturer(const char* course_id, const char* course_cs, int first_choose, int first_overflow) {
	std::ifstream inp(".\\layout\\minibox.layout");
	if (!inp.is_open()) {
		MessageBox(NULL, TEXT("minibox.layout is not exist"), TEXT("error layout"), MB_OK);
		return 0;
...
LAYOUT:
	...
	NO_CHANGE:
		...
		if (KEY_EDIT(c)) { npattendance::edit(course_id, course_cs, 10 + choose + overflow, editrow); goto LAYOUT; }
		if (c == 224 || c == 0) {
			c = getch();
			if (c == KEY_UP && choose > 0) { if (--choose + overflow < 0) overflow++; }
			else if (c == KEY_DOWN && choose < cur) { if (++choose < cur - 16) overflow--; }
			else if (c == KEY_LEFT) return 1;
			else if (c == KEY_RIGHT) { if (npscoreboard::lecturer(course_id, course_cs, choose, overflow) == 0) return 0; goto LAYOUT; }
			else goto NO_CHANGE;
			continue;
		}
		goto NO_CHANGE;
...
void npattendance::save(const char* course_id, const char* course_cs) {
...
void npattendance::edit(const char* course_id, const char* course_cs, int y, int i) {
